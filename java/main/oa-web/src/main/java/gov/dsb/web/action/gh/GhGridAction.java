...
public class GhGridAction extends PageActionSupport<Gh> {
...
//                "and true=all( select b.new from b where b.student = t )";
        ...
        String hql = "from Gh where ghType.id=?";
...
//            hql += " where endtime >= to_char(sysdate)";
        ...
        List<Gh> list = page.getResult();
        System.out.println("list.size() = " + list.size());

        if (page.getResult() != null) {
            Collection<DocDocumentAttach> attachs = new ArrayList<DocDocumentAttach>();
            for (Gh gh : page.getResult()) {
                if (gh.getDocdocument() != null) {
                    DocDocument docDocument = docDocumentDao.get(gh.getDocdocument().getId());
