...
//        var result = doPrivilege(privilegecode);
        function initTree() {
            ...
            //定义树的根节点
            var root = new Ext.tree.AsyncTreeNode({id:'root',text:'机关工会栏目',icon:iconPath + 'privilege_relate.gif'});

            //构造树面板 ，url="treedata.jsp"定义了异步数据源 .
            var treePanel = new TreePanel(root, null, "gh-type-tree!treedata", imageUrl, " ");
...
//                if (!result.s03_privilege_C) {
                    treeMenu.addItem(new MenuItem(node, "添加", "gh-type!input?parentid="));
                    treeMenu.addSeparator();
//                }
                treeMenu.addItem(new MenuItem(node, "刷新", updateChildren));
            } else if (node.id.indexOf("sys-privilege") >= 0) {
//                if (!result.s03_privilege_C) {
                    treeMenu.addItem(new MenuItem(node, "添加", "gh-type!input?parentid=" + getIdElement(id)));
                    treeMenu.addSeparator();
//                    treeMenu.addItem(new MenuItem(node, "添加操作", "gh-type!crudinput?parentid=" + getIdElement(id)));
//                    treeMenu.addSeparator();
//                }
//                if (!result.s03_privilege_U) {
                    treeMenu.addItem(new MenuItem(node, "更新", "gh-type!input?id=" + getIdElement(id)));
                    treeMenu.addSeparator();
//                }
//                if (!result.s03_privilege_D) {
                    treeMenu.addItem(new MenuItem(node, "删除", "gh-type!delete?id=" + getIdElement(id), true));
                    treeMenu.addSeparator();
//                }
                treeMenu.addItem(new MenuItem(node, "刷新", updateChildren));
            }
...
//                if(!result.s03_privilege_R){
                    TreeAction("gh-grid?typeid=" + getIdElement(id));
//                }
            }
        }
    </script>
