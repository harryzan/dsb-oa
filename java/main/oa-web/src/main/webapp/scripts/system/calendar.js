//document.writeln('<script language="JavaScript" src="/web/skins/js/calendar/core.js"></script>');

function calendar(input, format)
/**************************************************************
 *函数名称：calendar()
 *入口参数： button
                        input
 ...
 *使用方法：首先引用该脚本文件
 *<input name="test"  /><input type="button" onclick="calendar(test, 'datetime')"/>
 *作者：zyl
 *时间：2007-9-27
 */
{
...
strFrame='<style>';
strFrame+='INPUT.button{BORDER-RIGHT: #DBDBDB 1px solid;BORDER-TOP: #DBDBDB 1px solid;BORDER-LEFT: #DBDBDB 1px solid;';
strFrame+='BORDER-BOTTOM: #DBDBDB 1px solid;BACKGROUND-COLOR: #DBDBDB;font-family:宋体;}';
...
strFrame+=' function timeClick(td, timePart){';
strFrame+='	if (event.shiftKey){';
strFrame+='		if (timePart == "hour") {document.all.tmpSelectHourLayer.style.display="none";';
strFrame+='		parent.meizzTheHour = 0; parent.evaSetTime();}';
strFrame+='		else if (timePart == "minute") {document.all.tmpSelectMinuteLayer.style.display="none";';
strFrame+='		parent.meizzTheMinute = 0; parent.evaSetTime();}';
strFrame+='		if (timePart == "second") {document.all.tmpSelectSecondLayer.style.display="none";';
strFrame+='		parent.meizzTheSecond = 0; parent.evaSetTime();}}';
strFrame+='	else if (event.ctrlKey){';
strFrame+='		if (timePart == "hour") {document.all.tmpSelectHourLayer.style.display="none";';
strFrame+='		parent.meizzTheHour = 23; parent.evaSetTime();}';
strFrame+='		else if (timePart == "minute") {document.all.tmpSelectMinuteLayer.style.display="none";';
strFrame+='		parent.meizzTheMinute = 59; parent.evaSetTime();}';
strFrame+='		if (timePart == "second") {document.all.tmpSelectSecondLayer.style.display="none";';
strFrame+='		parent.meizzTheSecond = 59; parent.evaSetTime();}}';
strFrame+='	else{';
strFrame+='		if (timePart == "hour") {parent.tmpSelectHourInnerHTML(td.innerText.length==3?td.innerText.substring(0,1):td.innerText.substring(0,2));}';
strFrame+='		else if (timePart == "minute") {parent.tmpSelectMinuteInnerHTML(td.innerText.length==3?td.innerText.substring(0,1):td.innerText.substring(0,2));}';
strFrame+='		else if (timePart == "second") {parent.tmpSelectSecondInnerHTML(td.innerText.length==3?td.innerText.substring(0,1):td.innerText.substring(0,2));}}}';
strFrame+='</scr' + 'ipt>';
...
strFrame+='                <td width=16 align=center bgcolor=#99BBE8 style="font-size:12px;cursor: pointer;color: #ffffff" ';
strFrame+='					   onclick="parent.meizzPrevM()" title="前 1 月" ><b >&lt;</b></td>';
strFrame+='				   <td width=60 align="center" bgcolor="#99BBE8"  style="font-size:12px;cursor:pointer" ';
strFrame+='				       onmouseover="style.backgroundColor=\'#D2EEF4\'"';
strFrame+='					   onmouseout="style.backgroundColor=\'#99BBE8\'" ';
...
strFrame+='					   bgcolor="#99BBE8" onmouseover="style.backgroundColor=\'#D2EEF4\'" ';
strFrame+='					   onmouseout="style.backgroundColor=\'#99BBE8\'" ';
...
strFrame+='				   <td width=16 bgcolor=#99BBE8 align=center style="font-size:12px;cursor: pointer;color: #ffffff" ';
strFrame+='					   onclick="parent.meizzNextM()" title="后 1 月" ><b >&gt;</b></td>';
strFrame+='			   </tr>';
strFrame+='		   </table></td></tr>';
strFrame+='	   <tr><td width=142 height=18 >';
strFrame+='		   <table border=0 cellspacing=0 cellpadding=2 bgcolor=#CECBCE ' + (bMoveable? 'onmousedown="DragStart()" onmouseup="DragEnd()"':'');
strFrame+='				BORDERCOLORLIGHT=#63A3E9 BORDERCOLORDARK=#EFA684 width=140 height=20  style="cursor:' + (bMoveable ? 'move':'default') + '">';
...
strFrame+='    <table border=1 cellspacing=2 cellpadding=2 BORDERCOLORLIGHT=#ffffff BORDERCOLORDARK=#FFFFFF bgcolor=#fff8ec width=140 height=120 >';
...
strFrame+='        <td colspan=5 align=right style="color:#1B386D"><span onclick="parent.setNull()" style="font-size:12px;cursor: pointer"';
strFrame+='         onmouseover="style.color=\'#8E8E8E\'" onmouseout="style.color=\'#1B386D\'" title="将日期时间清除">清除</span>&nbsp;&nbsp;<span onclick="parent.meizzToday()" style="font-size:12px;cursor: pointer"';
strFrame+='         onmouseover="style.color=\'#8E8E8E\'" onmouseout="style.color=\'#1B386D\'" title="当前日期时间">当前</span>&nbsp;&nbsp;<span style="cursor:pointer" id=evaAllOK onmouseover="style.color=\'#8E8E8E\'" onmouseout="style.color=\'#CB580D\'"  onclick="parent.closeLayer()" title="关闭日历">关闭&nbsp;</span></td></tr>';
strFrame+='    </table></td></tr><tr ><td >';
strFrame+='        <table id=bUseTimeTable style="display:block" border=0 cellspacing=1 cellpadding=0 width=100%  bgcolor=#ffffff height=22 >';
strFrame+='          <tr bgcolor="#F7F7F7">';
strFrame+='             <td id=bUseTimeLayer width=30 align=center ';
//strFrame+='             style="cursor:pointer" title="点击这里启用/禁用时间"';
...
var todayColor = "#D1DFF0";
var selectDayColor = "#99BBE8";
...
function setday(tt,obj, useTime) //主调函数
{
...
//	outButton = (arguments.length == 1) ? null : th;	//设定外部点击的按钮
	...
	//判断是否使用时间,非严格验证
//	if (outObject.maxLength>10 || outObject.value.length > 10)
    if (useTime)
    {
		bUseTime=true;
		bImgSwitch();
...
function meizzWriteHead(yy,mm,ss)	//往 head 中写入当前的年与月
{
	...
	if (!bUseTime)
	{
		odatelayer.bUseTimeTable.style.display="none";
		//var element = window.frames.endDateLayer.document.getElementById("bUseTimeTable");
...
function meizzSetDay(yy,mm)	//主要的写程序**********
{
	...
	{
		//初始化边框
		da.borderColorLight="#CECBCE";
		da.borderColorDark="#CECBCE";
		da.style.color="#2F2F2F";
		if(i<firstday)		//上个月的部分
		{
			...
			else
			{
				...
				meizzWDay[i]==outDate.getDate())? selectDayColor :
				(((mm==1?yy-1:yy) == new Date().getFullYear() && (mm==1?12:mm-1) == new Date().getMonth()+1 &&
				meizzWDay[i] == new Date().getDate()) ? todayColor:"#ffffff");
				//将选中的日期显示为凹下去
...

		{
			...
			else
			{
				da.style.backgroundColor =(yy==outDate.getFullYear() && mm== outDate.getMonth() + 1 && meizzWDay[i]==outDate.getDate())?
					selectDayColor:((yy == new Date().getFullYear() && mm == new Date().getMonth()+1 && meizzWDay[i] == new Date().getDate())?
					todayColor:"#f5f5f5");
				//将选中的日期显示为凹下去
...
//当前月有多少天
Date.prototype.getDayNumberOfMonth = function(){
    var date = this;
    var startDate = new Date(this.getYear(),this.getMonth(),1);
    date = date.dateAdd('m',1);
...
Date.prototype.getWeeksOfMonth = function(){
    var date = this;
    var days = date.getDayNumberOfMonth();
    var startDay = this.getDay();
    var weeks = parseInt((days + startDay)/7);
