package gov.dsb.web.action.common.tree;
...
public class MultiSysUserTreeAction extends TreeActionSupport {
    ...
    public String treedata() throws Exception {
        ...
        if (id.equals("root")) {
            ...
            List<SysDept> sysdepts = service.findByCriteria(Restrictions.eq("parent.id", parentid));
            for (SysDept sysdept : sysdepts) {
                TreeNode treeNode = new TreeNode();
                treeNode.setText(sysdept.getName());
                if(sysdept.getChildren().size() > 0 || sysdept.getSysusers().size() > 0){
