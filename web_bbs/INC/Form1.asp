...
<%if membername="" then%>
        <tr bgcolor="<%=TablebodyColor%>">
          ...
          <td width="80%"><input name="username" value="<%if membername<>"" then%><%=htmlencode(membername)%><%else%><%if boardskin=2 then response.write "客人"%><%end if%>" >
          &nbsp;&nbsp;<a href=reg.asp><font color="<%=TableContentcolor%>">您没有注册？</font></a> 
        </td>
        </tr>
        <tr bgcolor="<%=TablebodyColor%>">
...
<%end if%>
        <tr bgcolor="<%=TablebodyColor%>">
          ...
          <td width="80%"><font color="<%=TableContentcolor%>"><input name="subject" size=60 maxlength=80 value="" >&nbsp;&nbsp;不得超过 50 个汉字</font>
...
<%end if%>
<%'=requote(con)%>
<TEXTAREA name="Content" cols="80" rows="12" wrap=VIRTUAL title="可以使用Ctrl+Enter直接提交贴子"  onkeydown=ctlent()>
<%
	if request("reply")="true" then
		content=reubbcode(con)
	    	...
	    	response.write "[quote][b]以下是引用[i]"&username&"在"&dateandtime&"[/i]的发言：[/b]" & chr(13)
...
%></TEXTAREA>
                ...
                <td valign=middle><font color="<%=TableContentcolor%>"><input type=checkbox name="signflag" value="yes" checked>是否显示您的签名？
               </font>
<BR><BR></td>
