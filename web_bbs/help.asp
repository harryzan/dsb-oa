...
<%if request("action")="" then%>
        ...
        <p>关于<a href=help.asp?action=announce>发帖子</a>的帮助</p>
        <p>关于<a href=help.asp?action=edit>修改帖子</a>的帮助</p>
...
%>
	...
	<p><b>发送消息</b>：登录用户方能发送消息，一种是自己填写收件人名称（该用户必须是论坛注册用户）和完整的标题和内容，内容支持ubb格式；另外一种是在论坛中查看帖子的时候直接给作者发送消息，需要填写内容同上。发送消息后，该消息内容同时存入你的发件箱中。
...
%>
	...
	<br>个性头像：所选择头像将在帖子中出现，可预览所有头像，可自行连接图片url
...
<P><B>签名档:</B><BR>如果你选中“发帖时加上签名档”选项，你每次发帖时系统会在最后自动加上你的签名档。你可随时通过个人档案来修改你的签名档。签名档的调用是在阅读帖子时即时进行的，所以，如果你修改了签名档，你发的所有帖子（包括修改以前的）所附带的都是你的新签名档。
<P><B>帖子回复的Email通知:</B><BR>如果论坛管理员已把邮件功能打开，你可选中“有人回复你帖子时收到Email通知”，这样，如果有人回复你的主题，系统会自动Email通知你。
<P><B>注意:</B><BR>除了发起新帖时可以省去主题描述项外，你要确保填写所有项已成功发出帖子。<BR>
...
<P><B>签名档</B><BR>你可编辑有个人特色的签名档，这样在每次发帖子的时候你可选择加上。系统设定签名档不能超过三行（每行以回车键为准），并且可以使用论坛（UBB)标签，但禁止使用HTML标签。
...
<P><font color=red>[URL=HTTP://WWW.DONGHAI-BRIDGE.COM.CN]</font><A HREF="www.donghai-bridge.com.cn">天兴洲工程建设信息网</A><font color=red>[/URL]</font>：有两种方法可以加入超级连接，可以连接具体地址或者文字连接。
<p><font color=red>[EMAIL]</font><A HREF="mailto:test@mmail.com">test@mmail.com</A><font color=red>[/EMAIL]</font>
<P><font color=red>[EMAIL=MAILTO:test@mmail.com]</font><A HREF="mailto:test@mmail.com">沙滩小子</A><font color=red>[/EMAIL]</font>：有两种方法可以加入邮件连接，可以连接具体地址或者文字连接。
<P><font color=red>[img]</font><img src=http://www.icsh.sh.cn/images/*.gif><font color=red>[/img]</font>：在标签的中间插入图片地址可以实现插图效果。
...
<%next%>
<p><%=grade(18)%>为版主或者管理员设定，可以进入特定版面。
<p><%=grade(19)%>为管理员设定，可以对论坛帖子进行管理。
<p><%=grade(20)%>总版主，拥有全部权限。
        <p>查看<a href=help.asp>所有帮助文件</a> 
<%
end sub
stats="浏览论坛帮助"
